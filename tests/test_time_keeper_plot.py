"""特定の筋書き(`plot_id`)の開始時刻から years 年ぶん進める `loop_time_for_plot`。"""
import pytest
...
def test_loop_time_for_plot_starts_at_plot_start_and_covers_years(session, monkeypatch):
    place = Location(name="村", kind="村", text="")
    session.add(place)
    session.flush()
    plot = Plot(location_id=place.id, text="筋書き", start=Stamp(2100, 4, 1), end=Stamp(2300))
    session.add(plot)
    session.commit()
    ...
    result = main.loop_time_for_plot(ai, plot.id, years=5)
...
def test_loop_time_for_plot_rejects_missing_plot(session):
    with pytest.raises(ValueError, match="見つからない"):
        main.loop_time_for_plot(MockAIClient(), 999)


def test_loop_time_for_plot_rejects_plot_without_start(session):
    place = Location(name="村", kind="村", text="")
    session.add(place)
    session.flush()
    plot = Plot(location_id=place.id, text="筋書き")
    session.add(plot)
    session.commit()

    with pytest.raises(ValueError, match="start が無い"):
        main.loop_time_for_plot(MockAIClient(), plot.id)
