"""ミーム(`meme`)。語(term)・oracle・人物の筋書きから抜き出して貯める。
...
from DEM.db.schema import Character, Meme, Oracle, Term
...
def _term(session, text="語の説明") -> Term:
    term = Term(name="語", kind="用語", text=text)
    session.add(term)
    session.commit()
    return term
...
def test_memes_are_drawn_out_of_terms_oracles_and_character_plots(session):
    term = _term(session)
    ...
    assert all(record.meme_seeded for record in (term, oracle, planned))
...
def test_unseeded_records_are_the_ones_not_flagged(session):
    seeded = _term(session, "一つ目")
    unseeded = _term(session, "二つ目")
    seeded.meme_seeded = True
    session.commit()

    rows = session.scalars(meme_query.unseeded_select(Term)).all()
