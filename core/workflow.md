...
置き場所の決まりは [core/chronicle.md](chronicle.md)。形は `tools/schema.py` が決めている。
...
| 7   | **語**                               | `novels/terms/**/<語>/term.md`                      |

**`terms/` は世界線でディレクトリを切らない。** どの世界線の語かは front matter の
`世界` が持つ。**`characters/` は世界線ではなく出身地で切る**（[core/chronicle.md](chronicle.md)）。

**5 は作品が決まってから。** 世界だけ作るあいだ、人物は要らない。
群としての振る舞い（4）と、一人ひとりの中身（5）を混ぜない（[core/chronicle.md](chronicle.md)）。
...
**AI の第一想起で埋めない**（[core/principles.md](principles.md)）。
...
- **数を本文に出さない。** 台帳の数は作者だけが見る（[core/chronicle.md](chronicle.md)）
- 途中で新しい設定が生まれたら、**本文にだけ書いて、メモに控える**。
  世界の側へ戻すのは 3 の仕事

## 2-4 推敲

[core/checklist.md](checklist.md) を上から順に当てる。
...
- 新しく出した語 → `novels/terms/<語>/term.md`（上位の語があればその中へ）
