...
<header>
  ...
  <label class="hint" id="yearbox">年 <input type="range" id="year"> <span id="yearv"></span> <label><input type="checkbox" id="allyears" checked>全期間</label></label>
...
const shownCharacters = () => DATA.characters.filter(c => !state.hidden.has(c.kind ?? ""));
...
function renderGraph() {
  ...
  for (const c of chars) {
    ...
    s += `<g class="node" data-id="${c.id}" style="cursor:pointer" opacity="${inYear(c, state.year) ? 1 : 0.35}">`;
