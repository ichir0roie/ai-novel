...
(id が既にあれば上書き、無ければ新規に作る。id も tag も無ければ自動採番
する)。テーブル名は置き場所(`worlds/{table_name}/`)から読み、列の値は
各ファイルの `# data` の json から読む。自己参照 FK を持つテーブル
(`export_db` 側の説明を参照)は、ディレクトリの入れ子を親子関係として
再帰的に読み戻す。
