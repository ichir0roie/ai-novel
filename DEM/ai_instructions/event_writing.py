...
EVENT_RECORD_INSTRUCTION = (
    ...
    "書くのは、この出来事で新しく起きた事象だけ。誰が、誰(どの個体)に対して、"
    "何を使って、具体的に何をしたか。相手はそれにどう応じたか。関わった人物が"
    "何に対してどう感じ、何を望み、何を恐れたか。その結果、前と後で何がどう"
    "変わったか(立場・所属・取り決め・資源の量・力関係・居場所など)と、"
    "そのうち何がこの先も残るか。これらを事実として、改行で区切った短い段落で"
    "書く。きっかけには、渡された場所・環境・資源・産業・人間関係・地名のうち"
    "少なくとも一つを名指しで使い、行動は人物と個体それぞれを主語に立てて"
    "一件ずつ分けて書く。分かるところは、数・期間・物の名前・場所の名前・"
    "人物の名前を具体的に書き込む。状況を雰囲気で漂わせず、事実と関係で書く。"
    "「対応を迫られた」「立場を守る行動をとった」「新たな局面を迎えた」の"
    "ように何をしたかが分からない抽象的な言い回しは使わず、実際に取った行動"
    "(要求した物の名前、動かした資源の量、出した命令の中身、移った先の場所"
    "など)を書く。"
    "次のものは text に入れない。"
    "出来事の名前(event_name)やその言い換え。"
    "「発端」「行動」「感情」「変化と影響」のような見出し・ラベル・番号付きの項目名。"
    "渡された直近の出来事(recent_events)の名前を写した文や、過去の出来事の"
    "内容の要約・振り返り(過去の経緯は、必要なら一文で触れるだけにする)。"
    "人物・個体・場所の text に既に書いてある説明の写し。"
    "同じ内容を言い方を変えて二度書くこと。"
    "全体で400〜800文字程度。"
)
...
EVENT_PROGRESSION_INSTRUCTION = (
    ...
    "前回までと似た内容・結末を、名前だけ変えてそのまま繰り返さない。"
    "個体ごとに渡す recent_events(その個体自身が場所を問わず関わった直近の"
    "出来事)と、進めたい筋書きごとに渡す recent_events(この場所と、その"
    "上位の場所で直近使われた出来事の名前)も同じ基準で見る。"
