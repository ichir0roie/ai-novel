...
引くものが何件もあるなら `batch` で、一度にまとめて流す。
往復も読み込みも一回で済む。

**SQL を組み立てない。** db に文を渡す入口は無い。引くのも直すのも
`tools/query.py` の関数（と、それを呼ぶ下の入口）を通す。渡すのは
**時刻と、レコードの id だけ**。足りない引き方が出てきたら、
`tools/query.py` に関数を足す。
...
| 設定の本文を直す     | `novel.py text <id> --file <本文>`（そのあと `save`）          |
| その場・その時を引く | `novel.py scene --place <場所> --time <年>`                     |
| その時の出来事を引く | `novel.py events --time <4354 または 4354/09/28>`               |
| その id に掛かる出来事 | `novel.py events --of <id>`                                  |
| id 一件を引く        | `novel.py show-id <id>`                                        |
...
- **置き場所がレコードの種類を決める**: 一つにつき一ファイル。データがレコード、
  その下が文章。雛形は `python3 tools/novel.py template --kind 人物`。
  どこに置くかで何のレコードかが決まる（chronicle.md）
- **参照はパスで書く**: レコードの id は置き場所そのもの（`novels/` からの相対パス、
  `.md` まで含む）。他のレコードを指す欄（`親` `場所` `人物` `個体` `分類` `所属`）には
  **その id を書く**（`worlds/SFファンタジー/ノウル/ナヘレ/抱き森/フリステ.md`）。
  **名前では書かない**——重なるし、名を変えれば指し先が黙って外れる。
  id 自体はデータに書かない（置き場所から入る）。
  分からなければ `python3 tools/novel.py show <名前>` で id を見る
- **行動は出来事の一種**: `events/` に置いた一件も出来事のレコードになる。
  `親` が掛かり先の出来事、`人物`／`個体` が誰の行動か。場所は掛かり先から継ぐので、
  `python3 tools/novel.py scene --place <場所> --time <年>` で、
  その場・その時の要素（場所・出来事・行動・人物・個体）が一度に出る
...
- **欄を勝手に足さない**: データの欄は `tools/schema.py` が一か所で決めている。
...
- **1 話 1 ファイル**: `novels/stories/<作品名>/episodes/<話数>.md`（`3.md`。**ゼロ埋めしない**）。
  ...
  **本文のファイルのデータ（`# data`）は `同期` だけ。** 話数はファイル名、題は先頭の
  見出しから台帳に入る。作品の `meta.md` のほうは普通にデータを持つ（chronicle.md）
